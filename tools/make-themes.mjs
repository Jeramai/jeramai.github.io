...
/* The secret theme is hand-built, not generated: period-correct beats procedural here. */
const SECRET = {
  id: 't1997',
  name: '1997',
  mood: 'retro',
  ...
  css: {
    ...
    '--border-w': '2px',
    '--radius': '0px',
    '--shadow': 'inset 2px 2px 0 #ffffff, inset -2px -2px 0 #5a5a5a',
    '--head-spacing': '0.04em',
...
const pair = (a, b) => contrast(hexToHsl(SECRET.css[a]), hexToHsl(SECRET.css[b]));
SECRET.checks = {
  ink: pair('--ink', '--panel'),
  inkDim: pair('--ink-dim', '--panel'),
  accent: pair('--accent', '--panel'),
  link: pair('--link', '--panel'),
  headInkA: pair('--head-ink', '--head-a'),
  headInkB: pair('--head-ink', '--head-b'),
  marquee: pair('--marquee-ink', '--marquee-bg'),
  marqueeSep: pair('--marquee-sep', '--marquee-bg')
};
...
const ts = [
  ...
  `export const secretTheme: Theme = { id: '${SECRET.id}', name: ${JSON.stringify(SECRET.name)}, mood: '${SECRET.mood}', arch: '${SECRET.arch}', seed: ${SECRET.seed}, crt: ${SECRET.crt} };`,
  '',
